...
func (cfg *apiConfig) handlerSetSellSlippage(ctx context.Context, b *bot.Bot, update *models.Update) {
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.CallbackQuery.Message.Message.Chat.ID,
		Text:   "Reply with your new setting for the Sell Slippage in % (1 - 75%). Whole numbers only",
	...
	cfg.intSeqMap[chatID(update.CallbackQuery.Message.Message.Chat.ID)] = &interactionSequence{
		funcSlice:   []interactionHandler{cfg.handlerProcessSellSlippage},
		retValues:   retValues,
		createdAt:   time.Now(),
		nextFuncIdx: 0,
	}
...
func (cfg *apiConfig) handlerProcessSellSlippage(ctx context.Context, b *bot.Bot, msg *models.Message) {
	...
	if ok, _ := regexp.MatchString(`^[0-9]*$`, msg.Text); valid || !ok {
		b.SendMessage(ctx, &bot.SendMessageParams{
			ChatID: msg.Chat.ID,
			Text:   "Invalid Sell Slippage percent. Integer percentage expected.",
	...
	if percent < 1 || percent > 75 {
		b.SendMessage(ctx, &bot.SendMessageParams{
			ChatID: msg.Chat.ID,
			Text:   "Invalid Sell Slippage percent. Valid percentage range is 1 to 75 inclusive.",
		})
		return
	}
	...
	err = cfg.DB.UpdateSellSlippage(ctx, database.UpdateSellSlippageParams{
		TelegramID:   telegramID,
		SellSlippage: int16(percent),
	...
	settings.SellSlippage = int16(percent)
	...
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: msg.Chat.ID,
		Text:   fmt.Sprintf("Sell Slippage set to: %s%%", msg.Text),
	})
}
