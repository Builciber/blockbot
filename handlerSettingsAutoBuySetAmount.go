...
func (cfg *apiConfig) handlerSetAutoBuyAmount(ctx context.Context, b *bot.Bot, update *models.Update) {
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.CallbackQuery.Message.Message.Chat.ID,
		Text:   "Reply with your new Auto Buy Amount in MON.",
		ReplyMarkup: models.ForceReply{
			ForceReply:            true,
			InputFieldPlaceholder: "0.69420",
			Selective:             false,
		},
	})
	...
	cfg.intSeqMap[chatID(update.CallbackQuery.Message.Message.Chat.ID)] = &interactionSequence{
		funcSlice:   []interactionHandler{cfg.handlerProcessAutoBuyAmount},
		retValues:   retValues,
		createdAt:   time.Now(),
		nextFuncIdx: 0,
	}
...
func (cfg *apiConfig) handlerProcessAutoBuyAmount(ctx context.Context, b *bot.Bot, msg *models.Message) {
	...
	if val, _ := strconv.ParseFloat(msg.Text, 64); val == 0.0 {
		b.SendMessage(ctx, &bot.SendMessageParams{
			ChatID: msg.Chat.ID,
			Text:   "auto buy amount cannot be zero",
			ReplyParameters: &models.ReplyParameters{
				MessageID:                msg.ID,
				AllowSendingWithoutReply: true,
			},
	...
	err = cfg.DB.UpdateAutoBuyAmount(ctx, database.UpdateAutoBuyAmountParams{
		TelegramID:    telegramID,
		AutoBuyAmount: pgtype.Numeric{Int: big.NewInt(intAmount), Exp: exp, Valid: true},
	...
	settings.AutoBuyAmount = pgtype.Numeric{Int: big.NewInt(intAmount), Exp: exp, Valid: true}
	...
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: msg.Chat.ID,
		Text:   fmt.Sprintf("Auto buy amount set to: %s MON", msg.Text),
	})
}
