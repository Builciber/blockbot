...
func (cfg *apiConfig) handlerProcessBuyX(ctx context.Context, b *bot.Bot, msg *models.Message) {
	...
	b.SendMessage(ctx, &bot.SendMessageParams{
		...
		Text:      fmt.Sprintf("Purchase successful: Bought *%v %s* for *%v MON*\n[View on the explorer](https://testnet.monadexplorer.com/tx/%s)", strings.Replace(displayDecimal(buyResult.BoughtAmount, 3), ".", "\\.", 1), escapeMarkdown(token.Symbol), strings.Replace(displayDecimal(msg.Text, 3), ".", "\\.", 1), buyResult.TxHash),
	})
