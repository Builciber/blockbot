...
func (cfg *apiConfig) handlerSetBuyButtonLeft(ctx context.Context, b *bot.Bot, update *models.Update) {
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.CallbackQuery.Message.Message.Chat.ID,
		Text:   "Reply with your new setting for the left Buy Button in MON.",
		ReplyMarkup: models.ForceReply{
			ForceReply:            true,
			InputFieldPlaceholder: "0.5",
			Selective:             false,
		},
	})
	...
	cfg.intSeqMap[chatID(update.CallbackQuery.Message.Message.Chat.ID)] = &interactionSequence{
		funcSlice:   []interactionHandler{cfg.handlerProcessBuyButtonLeft},
		retValues:   retValues,
		createdAt:   time.Now(),
		nextFuncIdx: 0,
	}
...
func (cfg *apiConfig) handlerProcessBuyButtonLeft(ctx context.Context, b *bot.Bot, msg *models.Message) {
	...
	if ok, _ := regexp.MatchString(`^[0-9]*(.)?[0-9]*$`, msg.Text); valid || !ok {
		b.SendMessage(ctx, &bot.SendMessageParams{
			ChatID: msg.Chat.ID,
			Text:   "Invalid left buy button value",
	...
	if val, _ := strconv.ParseFloat(msg.Text, 64); val == 0.0 {
		b.SendMessage(ctx, &bot.SendMessageParams{
			ChatID: msg.Chat.ID,
			Text:   "left buy button amount cannot be zero",
			ReplyParameters: &models.ReplyParameters{
				MessageID:                msg.ID,
				AllowSendingWithoutReply: true,
			},
	...
	err = cfg.DB.UpdateBuyButtonLeft(ctx, database.UpdateBuyButtonLeftParams{
		TelegramID:    telegramID,
		BuyButtonLeft: pgtype.Numeric{Int: big.NewInt(intAmount), Exp: exp, Valid: true},
	...
	settings.BuyButtonLeft = pgtype.Numeric{Int: big.NewInt(intAmount), Exp: exp, Valid: true}
	...
	b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: msg.Chat.ID,
		Text:   fmt.Sprintf("Left Buy Button value set to: %s MON", msg.Text),
	})
}
