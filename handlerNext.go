...
func (cfg *apiConfig) handlerNext(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	startIndex := userBalances.steps * 10
	endIndex := (userBalances.steps + 1) * 10
	/*if startIndex > len(userBalances.balances) {
		return
	}*/
	if endIndex > len(userBalances.balances) {
		if len(userBalances.balances)%10 == 0 {
			return
		}
		if len(userBalances.balances)%10 > 0 {
